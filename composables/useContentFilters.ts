...
export function useContentFilters() {
  ...
  async function blockAccount(targetUserId: string) {
    ...
    await supabase.from('follows')
      .delete()
      .or(`follower_id.eq.${user.value.id},following_id.eq.${user.value.id}`)
  ...
  async function updatePrefs(updates: Partial<ContentFilterPrefs>) {
    ...
    const { data } = await supabase.from('content_filter_prefs')
      .upsert({ user_id: user.value.id, ...updates })
      .select()
      .single()
  ...
  watch(user, (u: User | null) => { if (u) loadFilters() }, { immediate: true })
