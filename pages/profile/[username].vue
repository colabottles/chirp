<template>
  <div>
    ...
    <!-- Profile not found -->
    <div v-else-if="!profile && !loadingProfile" class="empty-state" role="alert">
    ...
    <!-- Profile header -->
    <article v-else-if="profile" :aria-label="`${profile.display_name}'s profile`">
      <!-- Banner -->
      <div class="profile-banner" aria-hidden="true">
        <img v-if="profile.banner_url" :src="profile.banner_url"
          :alt="`${profile.display_name}'s banner image`" />
      </div>
      ...
      <section class="profile-info-bar" aria-label="Profile information">
        <div class="profile-avatar-row">
          <div class="profile-avatar-large">
            <img v-if="profile.avatar_url" :src="profile.avatar_url"
              :alt="`${profile.display_name}'s avatar`" width="80" height="80" />
          ...
          <div>
            ...
            <button v-else-if="user" class="profile-follow-btn" :class="{ following: isFollowing }"
              :aria-label="isFollowing ? `Unfollow ${profile.display_name}` : `Follow ${profile.display_name}`"
        ...
        <h2 class="profile-display-name">
          ...
          <span v-if="profile.is_verified" class="post-verified-badge"
            :title="`${profile.display_name} is verified`">
        ...
        <div class="profile-meta" role="list">
          <div v-if="profile.location" class="profile-meta-item" role="listitem">
          ...
          <div v-if="profile.website" class="profile-meta-item" role="listitem">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16"
              fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
        ...
        <div class="profile-stats" role="list">
          <NuxtLink :to="`/profile/${profile.username}/following`" class="profile-stat-link"
            role="listitem">
            <span class="profile-stat-num">{{ profile.following_count }}</span>
            &nbsp;Following
          </NuxtLink>
          <NuxtLink :to="`/profile/${profile.username}/followers`" class="profile-stat-link"
            role="listitem">
            <span class="profile-stat-num">{{ profile.followers_count }}</span>
            &nbsp;Followers
          </NuxtLink>
        </div>
      </section>
      ...
      <!-- Posts -->
      <section :aria-label="`${profile.display_name}'s ${activeTab}`" aria-live="polite">
        ...
        <div v-else-if="userPosts.length === 0" class="empty-state">
          ...
          <p class="empty-state-text">
            {{ isOwnProfile ? "You haven't posted anything yet." : `${profile.display_name} hasn't
            posted yet.` }}
          </p>
        </div>
...
const isOwnProfile = computed(
  () => user.value && profile.value?.id === user.value.id
)

useHead(() => ({
  title: profile.value
    ? `${profile.value.display_name} (@${profile.value.username}) — Chirp`
    : 'Profile — Chirp',
}))
...
watch(username, () => fetchProfileData(), { immediate: true })
