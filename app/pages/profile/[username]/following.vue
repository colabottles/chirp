  ...
  <div>
    ...
    <div
      ...
      <div>
        ...
        <p style="color: var(--color-text-tertiary); font-size: var(--text-sm);">Following</p>
      </div>
    </div>
    ...
    <div v-else-if="following.length === 0" class="empty-state">
      ...
      <h2 class="empty-state-title">Not following anyone yet</h2>
      <p class="empty-state-text">When {{ isOwnProfile ? 'you follow' : `${displayName} follows` }}
        someone, they'll appear here.</p>
    </div>

    <!-- Following list -->
    <ul v-else class="follow-list" aria-label="Following">
      <li v-for="follow in following" :key="follow.following_id" class="follow-list-item">
...
useHead(() => ({
  title: displayName.value ? `${displayName.value} is following — Chirp` : 'Following — Chirp',
}))
...
async function fetchFollowing() {
  ...
  const { data } = await supabase
    .from('follows')
    .select('following_id, profile:profiles!following_id(id, username, display_name, avatar_url, bio)')
  ...
  if (currentUser.value?.id && following.value.length > 0) {
    const ids = following.value.map((f) => f.profile.id)
...
watch(username, fetchFollowing, { immediate: true })
</script>
