<template>
  <section class="composer" aria-label="Compose a new post">
...
const canSubmit = computed(() => content.value.trim().length > 0 && !isOverLimit.value && !isPosting.value)
...
function handleImageUpload(event: Event) {
  ...
  if (!file) return
  if (!file.type.startsWith('image/')) { errorMsg.value = 'Please select an image file.'; return }
  if (file.size > 5 * 1024 * 1024) { errorMsg.value = 'Image must be under 5MB.'; return }
...
async function handleSubmit() {
  ...
  try {
    ...
    if (imageFile.value) {
      ...
      const path = `posts/${session.value.user.id}/${Date.now()}.${ext}`
      const { error: uploadError } = await supabase.storage.from('post-images').upload(path, imageFile.value)
