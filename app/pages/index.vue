<template>
  <section aria-label="Home feed">
    ...
    <header class="feed-tabs">
      ...
      <div role="tablist" aria-label="Feed type" class="feed-tabs-list">
        <button
          id="tab-for-you"
          role="tab"
          class="feed-tab-btn"
    ...
    <PostComposer v-if="user" @posted="handleNewPost" />
    ...
    <section
      ...
      <article v-else-if="feedType === 'following' && posts.length === 0 && !loading"
        class="empty-state"
        ...
        <p class="empty-state-text">When you follow accounts, their posts will show up here.</p>
        <NuxtLink to="/explore" class="composer-submit-btn"
          ...
          Find people to follow</NuxtLink>
      </article>
      ...
      <section v-else aria-label="Posts" aria-live="polite" aria-atomic="false">
        ...
        <!-- End of feed -->
        <p v-if="!hasMore && posts.length > 0" class="empty-state"
          style="padding: var(--space-8); font-size: var(--text-sm);" role="status">
...
useHead({ title: 'Home — Chirp' })

const user = useSupabaseUser()
const feedType = ref<FeedType>('for-you')

const { posts, loading, loadingMore, hasMore, error, fetchPosts, prependPost, removePost } = useFeed(feedType)
const { onPosted } = useComposerModal()

onPosted((post) => prependPost(post))

function setFeed(type: FeedType) { feedType.value = type }
function handleNewPost(post: Post) { prependPost(post) }

function loadMore() {
  if (!loadingMore.value && hasMore.value) fetchPosts()
}
