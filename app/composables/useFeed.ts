...
export function useFeed(feedType: Ref<FeedType> | FeedType = 'for-you') {
  ...
  async function fetchPosts(reset = false) {
    ...
    try {
      ...
      if (session.value?.user?.id && postIds.length > 0) {
        const [{ data: likes }, { data: reposts }, { data: bookmarks }] = await Promise.all([
          supabase.from('likes').select('post_id')
            .eq('user_id', session.value.user.id).in('post_id', postIds),
          supabase.from('posts').select('repost_of_id')
            .eq('user_id', session.value.user.id).in('repost_of_id', postIds),
          supabase.from('bookmarks').select('post_id')
            .eq('user_id', session.value.user.id).in('post_id', postIds),
        ])
        likedIds = new Set(likes?.map((l) => l.post_id) ?? [])
        repostedIds = new Set(reposts?.map((r) => r.repost_of_id).filter((id): id is string => id !== null) ?? [])
  ...
  function subscribeToFeed() {
      ...
      const channel = supabase
        ...
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'posts' },
          async (payload) => {
            if (payload.new.user_id === session.value?.user?.id) return
  ...
  watch(type, () => fetchPosts(true))

  // Session watch handles initial load
  watch(session, (s) => {
    if (s !== undefined) fetchPosts(true)
  }, { once: true })
